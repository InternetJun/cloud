#参考文章
[数据库交互](https://blog.csdn.net/caplike/article/details/107059387)   
其实就是对客户端的一种封装处理，没有啥   
## 日志的相关
...
slfj是一个协议，是标准的接口，需要和log4j2\log4j等日志整合。
> 要完成日志的配置和对应的jar包依赖，应该要遵循如下的规则
~~~ xml
<dependency>
...
<Configuration status="INFO">
    <Appenders>
        <RollingFile name="RollingFile" fileName="logs/app.log"
                     filePattern="logs/app-%d{MM-dd-yy}.log.gz">
            <PatternLayout>
                <pattern>%d %p %C{1.} [%t] %m%n</pattern>
            </PatternLayout>
            <Policies>
...
~~~
## oauth  
在oauth中引入tls的安全认证的问题。    
[介绍加入安全验证处理](https://blog.csdn.net/caplike/article/details/108024763?spm=1001.2014.3001.5502)   
`待处理事项`：对客户端、资源、服务器的管理。   
refreshtoken是没有持有用户的信息，只是替换了原有有的值。
## 基本字符的处理
~~~java
/**
 * 
